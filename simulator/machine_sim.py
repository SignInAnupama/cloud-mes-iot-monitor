...
def write_to_influx(payload):
    point = (
        ...
        .field("cycle_time",     float(payload["cycle_time_sec"]))
        .field("temperature",    float(payload["temperature_c"]))
        .field("oee_pct",        float(min(payload["oee_pct"], 100.0)))
        ...
        .field("ideal_cycle",    float(payload["ideal_cycle_sec"]))
    )
